generate_placebos <-
  function(dataprep.out, synth.out, Sigf.ipop = 5) {
    ...
    names.and.numbers <- subset(dataprep.out$names.and.numbers, 
                                unit.numbers != tr)
    ...
    syn.plac <- function(i, dataprep.out) {
      ...
      d <- which(regexpr(tr, str_split(dataprep.out$tag$foo[id], 
                                       ", ")[[1]]) == T)
      for (j in 1:length(dataprep.out$tag$foo)) {
        foo[j] <- paste(str_split(dataprep.out$tag$foo[j], 
                                  ", ")[[1]][-c(d[1]:d[length(d)])], collapse = ", ")
      }
      ...
      dp <- list(X0 = X0, X1 = X1, Z0 = Z0, Z1 = Z1, Y0plot = Y0plot, 
                 ...
                 tag = tag)
      s.out <- synth(data.prep.obj = dp, Sigf.ipop=Sigf.ipop, optimxmethod = "All")
    ...
    for (i in 1:n) {
      ...
      b[, i] <- temp$a
      colnames(b)[i] <- paste("synthetic", as.character(names.and.numbers[i, 
                                                                          2]), sep = ".")
      mspe.placs[i, ] <- temp$s.mspe
    }
    df <- cbind(b, dataprep.out$Y0, dataprep.out$Y1, dataprep.out$Y0plot %*% 
                  synth.out$solution.w, dataprep.out$tag$time.plot)
    colnames(df)[(ncol(df) - 2):ncol(df)] <- c("Y1", "synthetic.Y1", 
                                               "year")
    t0 <- as.numeric(dataprep.out$tag$time.optimize.ssr[1])
    t1 <- as.numeric(dataprep.out$tag$time.optimize.ssr[length(dataprep.out$tag$time.optimize.ssr)]) + 
      1
    treated.name <- as.character(dataprep.out$names.and.numbers$unit.names[dataprep.out$names.and.numbers[, 
                                                                                                          2] %in% dataprep.out$tag$treatment.identifier])
    loss.v <- synth.out$loss.v
    res2 <- list(df = df, mspe.placs = mspe.placs, t0 = t0, t1 = t1, 
                 tr = tr, names.and.numbers = names.and.numbers, n = n, 
                 treated.name = treated.name, loss.v = loss.v)
    return(res2)
  }
