...
```{r}
denver_violent_prep <- dataprep(foo = reduced_database_violent, predictors = 
                                  c("population", # Removed population density because model gaps for more recent years are smaller that way.
                                    "female_household", 
                                                "hs", "res_stability", "over_18", 
                                                "white_percent", "self_employed", 
                                                "unemployment", 
         ...
         predictors.op = "mean", dependent = "violent_crime_rate", unit.variable = 
         "id", time.variable = "year", unit.names.variable = "NAME", 
         treatment.identifier = 26, controls.identifier = c(1:4, 6:19, 21:25, 27:34, 
                                                            36:102), 
         time.predictors.prior = 2011:2020, time.optimize.ssr = 2017:2019, 
         time.plot = 2011:2021)
...
cspd_violent_prep <- dataprep(foo = reduced_database_violent, predictors = 
                                  c("pop_density", "population", #Keep both because allows for longer ttracking 
                                    "female_household", 
                                                "hs", "res_stability", "over_18", 
                                                "white_percent", "self_employed", 
                                                "unemployment", 
         ...
         predictors.op = "mean", dependent = "violent_crime_rate", unit.variable = 
         "id", time.variable = "year", unit.names.variable = "NAME", 
         treatment.identifier = 20, controls.identifier = c(1:4, 6:19, 21:25, 27:34, 
                                                            36:102), 
         time.predictors.prior = 2011:2020, time.optimize.ssr = 2017:2020, 
         time.plot = 2011:2021)
...
```{r}
synth_cspd_violent <- synth(cspd_violent_prep)
```
...
aurora_violent_prep <- dataprep(foo = reduced_database_violent, predictors = 
         ...
         predictors.op = "mean", dependent = "violent_crime_rate", unit.variable = 
         "id", time.variable = "year", unit.names.variable = "NAME", 
         treatment.identifier = 5, controls.identifier = c(1:4, 6:19, 21:25, 27:34, 
                                                            36:102), 
         time.predictors.prior = 2011:2020, time.optimize.ssr = 2017:2020, 
         time.plot = 2011:2021)
...
denver_property_prep <- dataprep(foo = reduced_database_violent, predictors = 
         ...
         treatment.identifier = 26, controls.identifier = c(1:4, 6:19, 21:25, 27:34, 
                                                            36:102), 
         time.predictors.prior = 2011:2020, time.optimize.ssr = 2016:2019, 
         time.plot = 2011:2021)
...
```{r}
synth_cspd_property <- synth(cspd_property_prep)
```
...
aurora_property_prep <- dataprep(foo = reduced_database_violent, predictors = 
                                  c("population", "median_income", # standardized to population to match Denver
                                                "hs", "res_stability", "over_18", 
                                                "white_percent", "self_employed", 
                                                "unemployment", 
                                                "child_poverty"), 
...
mspe_ratio <- mspe(est, real) / synth_aurora_property$loss.v
...
```{r gen-placebos}
# placebos <- generate.placebos(denver_property_prep, synth_denver_property) ## Placebos
# saveRDS(placebos, 
       # file = "/Users/andrewqin02/Desktop/R/NPAP/data/placebos.Rdata")

placebos$mspe.placs <- replace(placebos$mspe.placs, placebos$mspe.placs < 0.1, 
                               161.6311) # Correct for error on Boca Raton, Florida, number obtained from manually running synth for Boca Raton with all optimization methods
```

```{r denver-placebo-results}
test_results <- mspe.test(placebos)$test
mspe.test(placebos, discard.extreme = TRUE, mspe.limit = 5)$p.val
```
...
P-value without removing jurisdictions with significantly greater MSPEs is 0.112.

```{r}
extremes <- which(placebos$mspe.placs >= 5 * l) # l = Aurora loss.v value 
unit.names <- placebos$names.and.numbers$unit.names[extremes]
