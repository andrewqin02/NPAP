...
reduced_database <- full_database %>% 
  ...
  filter(NAME != "Jurupa Valley, California") %>% #Jurupa Valley doesn't have land area data for some reason
...
I created a full model, incorporating every predictor outside of population below. 

```{r test-model}
diag1 <- lm(violent_crime_rate ~ pop_density + female_household + hs + res_stability + over_18 + white_percent +
   self_employed + unemployment + median_income + child_poverty + owner_occupied + year, data = reduced_database)
tidy(diag1)
```
As we can see, coefficients related to female household percentage, median income, and year were generally nonsignificant with p-values above 0.05. I used a backward selection process using AIC (erring towards the inclusion of variables to input into the synthetic control model) to determine what variables should be excluded.

```{r initial-model-step}
backward_diag1 <- step(diag1)
```

Through backward selection, I learned that year and median income should be removed from the analysis.
...
```{r test-model-log}
log_diag1 <- lm(log(violent_crime_rate) ~ pop_density + female_household + hs + res_stability + over_18 + white_percent +
   self_employed + unemployment + child_poverty + owner_occupied, data = reduced_database)
...
Model R-squared value is 0.571, indicating that roughly 57.1% of the variation in violent crime rates is explainable by the mentioned predictors. 
...
```{r}
diag2 <- lm(property_crime_rate ~ pop_density + female_household + hs + 
              res_stability + over_18 + white_percent +
   self_employed + unemployment + median_income + child_poverty + 
     owner_occupied, data = reduced_database)

backward_diag2 <- step(diag2)
```

The backward selection process removed racial homogeneity (percent of population who is white) and percentage of population who owns their owhn home (owner occupied housing percentage).
...
denver_violent_prep <- dataprep(foo = reduced_database_violent, predictors = 
         ...
         treatment.identifier = 73, controls.identifier = c(1:71, 74:207), 
