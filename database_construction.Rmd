...
prep <- dataprep(foo = no_colorado1, predictors = c("population", "female_household", 
         ...
         predictors.op = "mean", dependent = "violent_crime_rate", unit.variable = 
         "id", time.variable = "year", unit.names.variable = "NAME", 
         treatment.identifier = 186, controls.identifier = c(496, 449, 14, 266, 269), 
         time.predictors.prior = 2011:2019, time.optimize.ssr = 2014:2019, 
         time.plot = 2011:2019)
...
```{r}
synth_complete_test <- synth(prep)
```
...
```{r prep-co-springs}
prep_cosprings <- dataprep(foo = no_colorado1, predictors = c("population", 
                                                              "female_household", 
                                                "hs", "res_stability", "over_18", 
                                                "white_percent", "self_employed", 
                                                "unemployment", "median_income", 
...
synth_co_springs_test <- synth(prep_cosprings)
```
...
```{r}
synth_denver_property <- synth(denver_prop_prep_test)
```
...
14 - Fort Smith, Arkansas - 0.2095526
489 - Seattle, WA - 0.2004187
443 - Houston, TX - 0.1772544
266 - Champaign, IL - 0.1243610
326 - Ann Arbor, MI
...
denver_prop_prep_test <- dataprep(foo = no_colorado1, predictors = c("population", "female_household", 
         ...
         predictors.op = "mean", dependent = "property_crime_rate", unit.variable = 
         "id", time.variable = "year", unit.names.variable = "NAME", 
         treatment.identifier = 186, controls.identifier = c(466, 266, 512, 14, 449, 329), 
         time.predictors.prior = 2011:2018, time.optimize.ssr = 2016:2019, 
         time.plot = 2011:2019)
```
...
```{r}
weights_denver_prop <- synth_denver_property_test$solution.w
weights_denver_prop <- tibble(weight = weights_denver_prop, id = rownames(weights_denver_prop))
weights_denver_prop <- weights_denver_prop %>% 
  arrange(desc(weight)) %>% 
  mutate(id = as.numeric(id))
