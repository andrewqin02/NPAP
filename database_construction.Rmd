...
lm_database <- reduced_database %>% 
  filter(year != 2020) %>% 
  select(violent_crime_rate, property_crime_rate, pop_density, female_household, hs, res_stability, over_18, white_percent, self_employed, 
         unemployment, median_income, child_poverty, owner_occupied, year)
lm_database2 <- reduced_database %>% 
  filter(year == 2019)
```

```{r test-model}
diag1 <- lm(property_crime_rate ~ pop_density + female_household + hs + res_stability + over_18 + white_percent +
   self_employed + unemployment + median_income + child_poverty + owner_occupied + year, data = lm_database)
tidy(diag1)
```

```{r initial-model-step}
initial <- lm(violent_crime_rate ~ 1, data = lm_database)

backward <- step(diag1)
```

no median income and year

```{r}
diag2 <- lm(property_crime_rate ~ pop_density + female_household + hs + res_stability + over_18 + white_percent +
