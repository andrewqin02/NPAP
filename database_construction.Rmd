...
```{r test-model}
diag1_full <- lm(property_crime_rate ~ population + pop_density + hs + res_stability + over_18 + white_percent +
   self_employed + unemployment + median_income + child_poverty + owner_occupied + year, data = reduced_database_violent)
glance(diag1_full)
```
...
```{r}
denver_violent_prep2 <- dataprep(foo = reduced_database_violent2, predictors = 
...
```{r}
synth_denver_violent2 <- synth(denver_violent_prep2, optimxmethod = "All")
```

```{r}
path.plot(synth.res = synth_denver_violent2,
dataprep.res = denver_violent_prep2,
Ylab = c("Violent Crime Rates"),
Xlab = c("Year"),
Legend = c("Denver, CO","Synthetic Denver")
)
...
```{r}
cspd_violent_prep2 <- dataprep(foo = reduced_database_violent2, predictors = 
...
```{r}
synth_cspd_violent2 <- synth(cspd_violent_prep2, optimxmethod = "All")
```
...
```{r}
aurora_violent_prep2 <- dataprep(foo = reduced_database_violent2, predictors = 
                                  c("population", "pop_density", "owner_occupied",
                                                "hs", "res_stability", "over_18", 
                                                "white_percent", "self_employed", 
                                                "unemployment", "median_income",
...
```{r}
synth_aurora_violent2 <- synth(aurora_violent_prep2, optimxmethod = "All")
```
...
```{r}
synth_denver_property2 <- synth(denver_property_prep2, optimxmethod = "All")
```

```{r}
path.plot(synth.res = synth_denver_property2,
dataprep.res = denver_property_prep2,
Ylab = c("Property Crime Rates"),
Xlab = c("Year"),
Legend = c("Denver, CO","Synthetic Denver")
)
...
```{r}
synth_cspd_property2 <- synth(cspd_property_prep2, optimx = "All")
```
...
```{r}
synth_aurora_property2 <- synth(aurora_property_prep2, optimxmethod = "All")
```
...
```{r gen-placebos}
placebos9 <- generate.placebos(denver_violent_prep2, synth_denver_violent2) ## Placebos
saveRDS(placebos9, 
       file = "/Users/andrewqin02/Desktop/R/NPAP/data/placebos9.Rdata")

placebos10 <- generate.placebos(denver_property_prep2, synth_denver_property2)
saveRDS(placebos10, 
       file = "/Users/andrewqin02/Desktop/R/NPAP/data/placebos10.Rdata")

```

```{r denver-placebo-results}
test_results <- SCtools::mspe.test(placebos10)$test
SCtools::mspe.test(placebos10, discard.extreme = TRUE, mspe.limit = 5)$p.val
```

```{r denver-placebo-results}
test_results2 <- SCtools::mspe.test(placebos8)$test
SCtools::mspe.test(placebos8, discard.extreme = TRUE, mspe.limit = 5)$p.val
```
