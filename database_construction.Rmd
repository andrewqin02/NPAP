...
```{r}
synth_aurora_property <- synth(aurora_property_prep)
```
...
test_results <- test_results %>% 
  filter(MSPE.ratios < 10000) #filtering out one massive outlier at Boca Raton
...
test_property_prep <- dataprep(foo = reduced_database_violent, predictors = 
         ...
         predictors.op = "mean", dependent = "property_crime_rate", unit.variable = 
         "id", time.variable = "year", unit.names.variable = "NAME", 
         treatment.identifier = 1, controls.identifier = c(2:4, 6:19, 21:25, 27:34, 
                                                            36:102), 
         time.predictors.prior = 2011:2020, time.optimize.ssr = 2016:2019, 
         time.plot = 2011:2021)
...
```{r}
synth_test_property <- synth(test_property_prep, optimxmethod = "All")
```

```{r}
test_property_prep$Y0plot %*% synth_test_property$solution.w
```
...
denver_property_prep$Y0plot %*% synth_denver_property$solution.w
