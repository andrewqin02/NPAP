...
denver_violent_prep <- dataprep(foo = reduced_database_violent, predictors = 
         ...
         treatment.identifier = 25, controls.identifier = c(1:4, 6:9, 11:19, 21:24, 26:93, 
                                                            95:96), 
         time.predictors.prior = 2011:2020, time.optimize.ssr = 2011:2019, 
         time.plot = 2011:2021)
...
```{r gen-placebos}
placebos3 <- generate.placebos(denver_property_prep, synth_denver_property) ## Placebos
saveRDS(placebos2, 
       file = "/Users/andrewqin02/Desktop/R/NPAP/data/placebos3.Rdata")
```
...
```{r p-value-cspd}
extremes <- which(placebos2$mspe.placs >= 5 * l_cspd) # l = Aurora loss.v value 
unit.names <- placebos2$names.and.numbers$unit.names[extremes]
unit_names <- tibble(unit = unit.names) # Find unit names for those that qualify as extremes
