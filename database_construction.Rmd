...
```{r test-model}
diag1_full <- lm(violent_crime_rate ~ pop_density + female_household + hs + res_stability + over_18 + white_percent +
...
tidy(diag1_full)
```
As we can see, coefficients related to female household percentage, median income, and year were generally nonsignificant with p-values above 0.05. I used a backward selection process using BIC to determine what variables should be excluded.


```{r initial-model-step}
backward_diag1 <- step(diag1_full, direction = "backward", k = log(2533)) # with BIC
forward_diag1 <- step(diag1_null, direction = "forward", scope = list(
  lower = diag1_null, upper = diag1_full), k = log(2533))
```

Both backward and forward selection concluded that child poverty, high school education, percentage of residents who are white, percent of owner-occupied buildings, unemployment rate, and rate of self-employed individuals should be kept.
...
```{r test-model-log}
log_diag1 <- lm(log(violent_crime_rate) ~ child_poverty + hs + owner_occupied 
                + self_employed + white_percent + unemployment, data = reduced_database)
...
aurora_violent_prep <- dataprep(foo = reduced_database_violent, predictors = 
         ...
         time.predictors.prior = 2011:2018, time.optimize.ssr = 2014:2017, 
         time.plot = 2011:2019)
