...
placebos5 <- readRDS("data/placebos5.Rdata") # Property crime version
placebos6 <- readRDS("data/placebos6.Rdata") # Violent crime version

test_results <- mspe.test(placebos5)$test
test_results2 <- mspe.test(placebos6)$test # set up property crime one here as well
```
...
extremes <- which(placebos5$mspe.placs >= 5 * l_cspd) # l = CSPD loss.v value
unit.names <- placebos5$names.and.numbers$unit.names[extremes]
...
extremes <- which(placebos5$mspe.placs >= 5 * l) # l = Aurora loss.v value 
unit.names <- placebos5$names.and.numbers$unit.names[extremes]
unit_names <- tibble(unit = unit.names) # Find unit names for those that qualify as extremes
...
As the histogram displays, most placebo MSPE ratios are centered at 0-10 with the exception of two placebo MSPE ratios above 150. Those MSPE ratios represent Evansville and Manchester and likely occurred from an exceptionally good fit with the data in pretreatment years with some declining fit in 2020. The outlier will be further explored in Appendix 2; however, we do not see a good reason to remove the outlier, as removing the outlier would not change the significance of any of our results.
...
ggplot(aes(x = MSPE.ratios)) + 
  ...
  geom_vline(xintercept = mspe2, color = "steelblue") + 
  geom_vline(xintercept = mspe1) + 
  labs(title = "Distribution of Placebo MSPE Ratios without Outliers", 
       x = "MSPE Ratios", 
       y = "Count") # Denver's value
```
...
l_cspd <- l_cspd[1]
extremes <- which(placebos6$mspe.placs >= 5 * l_cspd) # l = Aurora loss.v value 
unit.names <- placebos6$names.and.numbers$unit.names[extremes]
unit_names <- tibble(unit = unit.names) # Find unit names for those that qualify as extremes
...
l <- l[1]
extremes <- which(placebos6$mspe.placs >= 5 * l) # l = Aurora loss.v value 
unit.names <- placebos6$names.and.numbers$unit.names[extremes]
unit_names <- tibble(unit = unit.names) # Find unit names for those that qualify as extremes
