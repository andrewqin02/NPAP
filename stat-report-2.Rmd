...
*SKIP THE MODEL SELECTiON STUFF - we excluded certain variables after finding out that they worsened the synthetic control models. Give the example of single female led household percentage worsening the model (not that hard to get that example) and describe in an Appendix format! Also we'll generate placebos with all of them anyway j to see what happens so that should be in the same appendix with a table!!!*
...
The results of this methodology, in their entirety, will be described in Appendix __.
...
## Appendix 1: Model Selection Processes to Determine Value of Predictors

This Appendix describes the process by which we removed certain predictors from the analysis based on model selection. We chose to use backward selection to determine the variables we should remove from the analysis, as we presumed that all variables were good predictors based on previous scholarly evidence unless the model selection process found them to be unhelpful. We also chose to use AIC as opposed to BIC to err towards the inclusion of variables to allow the synthetic control method to create jurisdictions that matched the observed jurisdictions in more dimensions, even if such dimensions weren't directly relevant enough to pass BIC model selection.

We began with violent crime. 

```{r test-model}
diag1_full <- lm(violent_crime_rate ~ population + pop_density + female_household + hs + res_stability + over_18 + white_percent + self_employed + unemployment + median_income + child_poverty + owner_occupied + year, data = reduced_database_violent)

diag1_null <- lm(violent_crime_rate ~ 1, data = reduced_database_violent)
...
Over_18 and female_household were excluded from the model.

```{r test-model}
diag1_full <- lm(property_crime_rate ~ population + pop_density + female_household + hs + res_stability + over_18 + white_percent + self_employed + unemployment + median_income + child_poverty + owner_occupied + year, data = reduced_database_violent)

backward_diag1 <- step(diag1_full, direction = "backward") # with AIC
```

Remove female_household, res_stability,self_employed, median_income - keep owner_occupied
