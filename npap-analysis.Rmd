...
daily_violent <- co_offenses %>% 
  filter(violent == 1) %>% 
  count(date)

monthly_violent <- co_offenses %>% 
  filter(violent == 1) %>% 
  count(my)
...
wa_offenses <- wa_offenses %>% 
  mutate(violent = ifelse(offense == "Aggravated Assault" | 
           ...
           offense == "Rape" | 
           offense == "Robbery" |
           offense == "Sexual Assault With An Object", 1, 0)) %>% 
  mutate(property = ifelse(offense == "All Other Larceny" | 
                             ...
                             offense == "Destruction/Damage/Vandalism of Property", 1, 0))
...
co_property_period <- co_offenses %>% 
  filter(property == 1) %>% 
  count(date) %>% 
  mutate(time_period = case_when(
    date >= as.Date("2019-06-19") & date < as.Date("2020-05-20") ~ "2019-20",
...
t1 <- co_property_period %>% 
  filter(time_period == "2019-20") %>% 
  select(n) %>% 
  rename("p1" = n)
t2 <- co_property_period %>% 
  filter(time_period == "2020-21") %>% 
  select(n) %>% 
  rename("p2" = n)
