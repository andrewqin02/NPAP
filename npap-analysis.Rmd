...
co_monthly_violent <- co_offenses %>% 
  ...
  count(month, year, my) %>% 
  mutate(time_period = case_when(
    my >= as.Date("2019-07-01") & my < as.Date("2020-07-01") ~ "2019-20",
    my >= as.Date("2020-07-01") & my < as.Date("2021-07-01") ~ "2020-21"
  )) %>% 
  filter(is.na(time_period) == FALSE)
...
Seattle, Washington (496) - 0.2174257
Houston, TX (449) - 0.2064292
Fort Smith, AR (14) - 0.1988124
Champaign, IL (266) - 0.1385536

```{r}
0.2174257 / (0.2174257 + 0.2064292 + 0.1988124 + 0.1385536)
0.2064292 / (0.2174257 + 0.2064292 + 0.1988124 + 0.1385536)
0.1988124 / (0.2174257 + 0.2064292 + 0.1988124 + 0.1385536)
0.1385536 / (0.2174257 + 0.2064292 + 0.1988124 + 0.1385536)
```
...
synthetic <- tibble(combined_analysiswa, combined_analysistx, combined_analysisfspd, 
                    combined_analysisil) %>% 
  mutate(diff = seattle_diff * 0.2856276 + tx_diff * 0.2711817 + fspd_diff * 0.2611757 + 
           il_diff * 0.182015) %>% 
  mutate(jurisdiction = "Synthetic Control")  %>% 
  select(diff, jurisdiction)

combined_synthetic <- full_join(combined_analysis, synthetic)
```
...
```{r}
0.2004187 / (0.2095526 + 0.2004187 + 0.1772544 + 0.1243610)
0.2095526 / 
```
...
wat1 <- wa_limited_property %>% 
  filter(time_period == "2019-20") %>% 
  select(n) %>% 
  rename("p1" = n)

wat2 <- wa_limited_property %>% 
  filter(time_period == "2020-21") %>% 
  select(n) %>% 
  rename("p2" = n)
...
wat1 <- fspd_limited_property %>% 
  filter(time_period == "2019-20") %>% 
  select(n) %>% 
  rename("p1" = n)

wat2 <- fspd_limited_property %>% 
  filter(time_period == "2020-21") %>% 
  select(n) %>% 
  rename("p2" = n)
...
wat1 <- il_limited_property %>% 
  filter(time_period == "2019-20") %>% 
  select(n) %>% 
  rename("p1" = n)

wat2 <- il_limited_property %>% 
  filter(time_period == "2020-21") %>% 
  select(n) %>% 
  rename("p2" = n)
